...
Now it's time to bring your agent to life and send it a query[^1]. To do this, you need a [`Runner`](https://google.github.io/adk-docs/runtime/), which is the central component within ADK that acts as the orchestrator. It manages the conversation, sends our messages to the agent, and handles its responses.
****
> [!tip] 
> תחום ה-AI Agents הוא חדש ומתפתח, והמושג "Runner" (המנוע שמריץ את הסוכן) מקבל פרשנות שונה אצל כל אחד מהמתחרים הגדולים של Google ADK.
> 
> בעוד ש-ADK משתמש במושג Runner מפורש עם "לולאת אירועים" (Event Loop), המתחרים פותרים את בעיית הניהול והתזמור (**Orchestration**) בדרכים אחרות:

|**מסגרת עבודה (Framework)**|**מה מחליף את ה-Runner?**|**הקונספט המרכזי**|
|---|---|---|
|**Google ADK**|**Runner**|לולאת אירועים (Event Loop) שמנהלת מצב וכלים.|
|**LangGraph**|**Graph Executor**|מעבר בין צמתים בגרף (State Machine).|
|**Semantic Kernel**|**Kernel / Planner**|ביצוע רציף של תוכנית (Sequential Pipeline).|
|**CrewAI**|**Process**|ניהול צוות – סוכן מנהל סוכן או משימה רודפת משימה.|
|**OpenAI**|**Run Object**|תהליך שרץ בענן (Server-Side Polling).|
...
> [!tip] 
> זה כלי חזק וחשוב בקורס ואני רוצה לראות שאני מצליח לעבוד איתו - במה הוא חזק. להבנתי אפשר להבין באופן ברור יותר מה קורה במנוע של הסוכן AGENT ולהשפיע עליו
